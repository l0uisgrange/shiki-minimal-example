...
<div class="items-stretch md:flex">
	<main class="min-h-dvh p-5 pt-10 md:px-8 md:py-10">
		...
		<div class="mb-4 grid gap-7 md:grid-cols-2">
			<div>
				<div
					class="dark:border-neutral-border mb-4 flex max-h-40 items-center overflow-hidden rounded-lg border border-neutral-200"
				>
		...
		<Notice type="warning" title="Warning">
			The two-node placement method is <span class="font-medium"
				>only suitable for components that can be represented with two connections</span
			>. For example, a MOSFET or BJT transistor requires three nodes, so it can only be
			placed using the one-node placement method.
		</Notice>
		...
		<p>
			Sometimes, you just want to connect a node from one component to another without
			worrying about coordinates or doing mental math. That’s where named anchors come in.
		</p>
		<p>
		...
		<p>
			Check the <a
				href="https://cetz-package.github.io/docs/basics/coordinate-systems/#relative"
